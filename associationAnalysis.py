...
def associationRules(categorical_values, paragraph_objects):
    ...
    for cluster in df['clusterN'].unique():
        ...
        # Check if there are transactions for the cluster
            ...
            if not rules.empty:
                ...
                filtered_rules = filtered_rules[
                    filtered_rules['consequents'].apply(
                        lambda x: len(list(x)) == 1 and list(x)[0].split('/')[0] == 'clusterN')]
                ...
                if not filtered_rules.empty:
                    ...
                    for paragraph_obj in paragraph_objs:
                        paragraph_obj.ruleA = [antecedent_lists, ['clusterN']]
        else:
            print("No transactions for cluster", cluster)

    for paragraph in paragraph_objects:
        print("Cluster Number:", paragraph.clusterN)
        print("Content:", paragraph.content)
        print("RuleA:", paragraph.ruleA)
        print()
'''
...
def associationRules(categorical_values, paragraph_objects):
    ...
    '''
    # Print the updated paragraph_objects list
    for paragraph in paragraph_objects:
        print("Cluster Number:", paragraph.clusterN)
        print("Content:", paragraph.content)
        print("RuleA:", paragraph.ruleA)
